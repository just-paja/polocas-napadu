...
import { createRedirectApp } from '../redirects.ts'

describe('redirects app', () => {
    let server
    
    beforeAll(async () => {
        server = createServer(createRedirectApp(JSON.stringify([
            {
        ...
        server.listen()
        server.origin = `http://[${server.address().address}]:${server.address().port}`
    })
    ...
    describe('given redirect is *.polocasnapadu.cz -> $1.polocas-napadu.cz', () => {
        ...
        it('translates internal.polocasnapadu.cz/ to internal.polocas-napadu.cz/', async () => {
            ...
            expect(res.headers.get('location')).toEqual('http://internal.polocas-napadu.cz/')
        })
        ...
        it('translates ukoly.polocasnapadu.cz/foo/bar to ukoly.polocas-napadu.cz/foo/bar', async () => {
            ...
            expect(res.headers.get('location')).toEqual('http://ukoly.polocas-napadu.cz/foo/bar')
        })
        
        it('keeps query string', async () => {
            const res = await fetch(`${server.origin}/foo/bar?active=1&sort=createdAt`, {
                method: 'GET',
                headers: { host: 'ukoly.polocasnapadu.cz' },
                redirect: 'manual',
            })
            expect(res.headers.get('location')).toEqual('http://ukoly.polocas-napadu.cz/foo/bar?active=1&sort=createdAt')
        })
    })
})